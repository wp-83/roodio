...
# Matikan warning agar log server bersih
warnings.filterwarnings('ignore')

def get_stats(feature_array, name):
    """
    Helper untuk menghitung Mean & Variance dari array fitur.
    """
...
def extract_features_from_file(file_path):
    ...
    """
    try:
        # Load 30 detik pertama (Konsisten dengan training)
        y, sr = librosa.load(file_path, duration=30, sr=22050)
        
        # Validasi file audio rusak/kosong
        if len(y) < sr:
            return None
