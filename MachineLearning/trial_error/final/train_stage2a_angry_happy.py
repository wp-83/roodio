...
for fold, (train_idx, test_idx) in enumerate(skf.split(X_audio_features, y_labels)):
    ...
    # Base Model: Audio (Random Forest n=100)
    # clf_audio = RandomForestClassifier(n_estimators=100, random_state=SEED)
    clf_audio = RandomForestClassifier(
        n_estimators=200,      # Tambah jumlah pohon biar voting lebih stabil
        max_depth=5,           # Batasi kedalaman pohon (jangan biarkan terlalu kompleks)
        min_samples_leaf=4,    # Minimal sampel di daun (mencegah hafalan data spesifik)
        max_features='sqrt',   # Standar yang bagus
        random_state=SEED
    )
...
if clf_audio_final is not None and meta_clf_final is not None:
    ...
    print("✅ Model Stage 2A (Angry/Happy) berhasil disimpan ke folder 'models/'")
else:
