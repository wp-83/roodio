...
print("🚀 EXP 45: HYPERPARAMETER TUNING CHAMPION")
print("   (GridSearch Optimization for 5 Models)")
...
# Ini adalah "Menu" yang akan dicoba satu per satu oleh AI
...
for name, model in base_models.items():
    ...
    # 2. Grid Search CV
    # Kita cari setting terbaik menggunakan Cross Validation
    grid = GridSearchCV(
        estimator=pipeline,
    ...
    best_params = str(grid.best_params_).replace('model__', '') # Cleanup string
    
    # Shorten param string for display
    display_params = (best_params[:28] + '..') if len(best_params) > 28 else best_params
    
    print(f"{name:<20} | {display_params:<30} | {best_acc:.2f}%")
...
print(f"   Accuracy: {champion['accuracy']:.2f}%")
...
# Note: GridSearch automatically refits the best model on the full dataset
# So 'final_model' is already trained on X, labels with best params

os.makedirs('models', exist_ok=True)
joblib.dump(final_model, 'models/stage2b_tuned_champion.pkl')

# Save metadata
with open('models/tuned_model_info.txt', 'w') as f:
    f.write(f"Champion: {champion['name']}\n")
    f.write(f"Accuracy: {champion['accuracy']:.4f}\n")
    f.write(f"Params: {champion['best_params']}\n")
