...
print(f"🚀 MEMULAI EXP 12: STAGE 1 + AUGMENTASI (3x DATA)...")
...
def extract_features_from_audio(y, sr=16000):
    """Ekstrak fitur langsung dari array audio (bukan file path)"""
    try:
        # 1. YAMNet Embedding
        # Normalize & Pad
...
# Kita load audio mentahnya ke RAM supaya bisa diaugmentasi nanti
print("📥 Loading Raw Audio ke Memory (Trimmed)...")

audio_data = [] # List of numpy arrays
labels = []
...
for mood in moods:
    ...
    for f in tqdm(files, desc=f"Load {mood}"):
        ...
        try:
            # Load & Trim di awal biar hemat RAM
...
for fold, (train_idx, test_idx) in enumerate(skf.split(audio_data, y_encoded)):
    ...
    print("   🔨 Augmenting & Extracting Training Data...")
    X_train_vec = []
    y_train_vec = []
...
# --- 6. FINAL RESULT ---
print("\n" + "="*50)
print("📊 HASIL AKHIR STAGE 1 (AUGMENTED)")
print("="*50)
...
plt.title(f'Binary Arousal (Augmented 5-Fold)\nAvg Acc: {avg_acc:.2f}%')
plt.ylabel('True'); plt.xlabel('Predicted')
plt.savefig('cm_exp12_stage1_augmented.png')
plt.show()