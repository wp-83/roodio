@php
    ...
    $borderMood = $styles['borderMood'];
    $textMood = $styles['textMood'];
    $bgMoodStyle = $styles['bgMoodStyle'];
    $elementColor = $styles['elementColor'];
    $scrollbarTheme = $styles['scrollbarTheme'];
    $borderTextareaStyle = $styles['borderTextareaStyle'];

    $creator = $thread->user->userDetail->fullname;
    $profilePicture = $thread->user->userDetail->profilePhoto;
...
<div class='{{ $bgContainer[$mood] }} relative rounded-lg h-max p-5 w-full overflow-hidden lg:max-w-md'>
    ...
    <div class='w-full relative'>
        ...
        @if ($isReplyable)
            {{-- PENTING: wire:ignore.self Mencegah Livewire menutup kembali div ini saat render ulang --}}
            <div wire:ignore.self class='mt-6 w-full hidden' id="reply-{{ $thread->id }}">
                <hr class='border border-shadedOfGray-30 my-2'>

                {{-- Reply Container --}}
                <div x-on:reply-posted.window="
        if ($event.detail.threadId == '{{ $thread->id }}') {
            $nextTick(() => {
                $el.scrollTop = $el.scrollHeight;
            });
        }
    " class="h-max max-h-64 mt-3 overflow-y-auto replyContainer font-secondaryAndButton scrollbar-thin {{ $scrollbarTheme[$mood] }}">
                    @forelse($thread->replies as $reply)
                        @php
                            $replyPhoto = $reply->user->userDetail->profilePhoto;
                            $replyName = $reply->user->userDetail->fullname;
                        @endphp
                        <div class='mb-3 flex flex-row items-start gap-3'>
                            ...
                            <div class='flex flex-col gap-1.25 w-full'>
                                <div>
                                    <p class='text-small font-bold {{ $textMood[$mood] }}'>{{ ($mainUser->id == $reply->userId) ? 'You' : $replyName }}</p>
                ...
                <div class="w-full mt-4 relative">
                    <form wire:submit.prevent="postReply">
                        <textarea
                            ...
                            class="font-secondaryAndButton text-small w-full min-h-1 max-h-18 p-2 pl-6 pr-20 py-3 overflow-y-auto resize-none border {{ $borderTextareaStyle[$mood] }} bg-shadedOfGray-10/60 not-placeholder-shown:bg-accent-85/10 rounded-3xl scrollbar-none placeholder:italic"
                        ...
                        <div class='absolute top-1 right-2'>
                            {{--
                                1. Input Submit Hidden (Trigger)
                                Gunakan ID unik per thread agar tidak salah sasaran
                            --}}
                            <input type="submit" id="submit-{{ $thread->id }}" class="hidden" />

                            {{--
                                2. Label sebagai Visual Button
                                Saat diklik, label ini memicu input submit di atas.
                                Kita bungkus x-button di sini.
                            --}}
                            <label for="submit-{{ $thread->id }}" class="cursor-pointer block transform scale-75 origin-top-right">
                                {{-- pointer-events-none agar klik tembus ke label --}}
                                <div class="pointer-events-none">
                                    <x-button :mood='$mood' content='Send' />
                                </div>
                            </label>
                        </div>
                    </form>
                    ...
                    @error('content')
                        <p class='font-secondaryAndButton error-message pt-0.1 mb-2 text-secondary-angry-100 text-xs'>{{ $message }}</p>
                    @enderror
                </div>
            </div>
        @endif
