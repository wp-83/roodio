...
from sklearn.svm import SVC  # <--- SOLUSI 3: SVM
...
print(f"🚀 MEMULAI EXP 29-LITE: SIMPLIFIED NN + SVM (NO AUGMENTATION)...")
...
def extract_audio(path):
    try:
        ...
        # Kita tetap pakai fitur tambahan (Std) biar NN punya info cukup
...
# --- SOLUSI 2: MODEL NEURAL NETWORK (SIMPLIFIED) ---
def create_tuned_model(input_shape):
    model = Sequential([
        # Arsitektur lebih ramping (128 neuron)
        Dense(128, activation='relu', input_shape=(input_shape,)),
        BatchNormalization(),
        Dropout(0.5), # Dropout agresif untuk cegah hafalan
    ...
    # Learning rate pelan tapi stabil
...
print(f"\n🚀 START TUNING (Simplified NN + SVM)...")
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X_audio_features, y_labels)):
    ...
    # --- STEP 1: TRAIN NN (TANPA AUGMENTASI) ---
    ...
    # Langsung train pakai data asli
    model_nn.fit(X_aud_tr, y_tr, 
                 epochs=200, 
                 batch_size=8, # Batch size kecil (8) bagus untuk data sedikit
    ...
    # SVM Setup: C=1.5 (Agak strict), RBF Kernel
...
print("\n" + "="*50)
print("📊 HASIL AKHIR (SIMPLIFIED NN + SVM)")
print("="*50)
