...
print("🚀 EXP 48 (RE-RUN): WINNING PARAMETERS ONLY")
print("   Goal: Verify the 89.75% Score Quickly")
...
# 🔥 PENTING: Sorting agar urutan file konsisten 100%
all_files.sort()
...
# Pastikan sorting stabil saat selecting top files
...
# ================= 4. DEFINE PIPELINE & WINNING PARAM =================
print("\n🔨 Configuring Grid Search (Winning Params Only)...")

# Base Pipeline
pipeline = Pipeline([
    ('scaler', StandardScaler()), 
    # Pastikan Estimator RFE juga punya SEED agar fitur yg dipilih sama
    ('selection', RFE(estimator=RandomForestClassifier(n_estimators=50, random_state=SEED))), 
    ('model', SVC(probability=True, random_state=SEED)) 
])

# 🔥 PARAMETER GRID JUARA (Single Combination)
param_grid = [
    {
        'selection__n_features_to_select': [30], # Top 30 Fitur
        'model__kernel': ['linear'],             # Kernel Linear
        'model__C': [0.5]                        # C = 0.5
    }
]
...
best_std = grid.cv_results_['std_test_score'][0] * 100

print("\n" + "="*50)
print(f"🏆 CHAMPION VERIFIED")
print("="*50)
...
with open('models/stage2b_info.txt', 'w') as f:
    f.write(f"Model: SVM Tuned Pipeline (Verified)\n")
    f.write(f"Accuracy: {best_acc:.2f}%\n")
    f.write(f"Std Dev: {best_std:.2f}%\n")
    f.write(f"Params: {grid.best_params_}\n")
