...
<div class='min-w-5 min-h-5 w-full h-max p-4 flex flex-row justify-start gap-3 rounded-md relative overflow-hidden duration-200 group {{ $bgSoftOpacityMood[$mood] }}'>
    <div class='h-26 w-26 shrink-0 relative z-100 rounded-sm object-cover overflow-hidden {{ $bgBasedMood[$mood] }}'>
    ...
    <div class='font-secondaryAndButton flex flex-col justify-between relative z-1000 w-full'>
        <div>
            <p class='text-body-size font-bold {{ $textMood[$mood] }}'>{{ Str::upper($playlistName) }}</p>
            <p class='text-small text-primary-70'>{{ $playlistDesc }}</p>
        </div>
        <div class='flex flex-row h-max items-center gap-2 text-micro'>
    ...
    <img src="{{ asset('assets/moods/icons/' . $mood . '.png') }}" alt="$mood" class='w-44 h-44 opacity-10 absolute right-0 top-0 translate-x-12 -rotate-30 translate-y-4 group-hover:opacity-20'>
</div>