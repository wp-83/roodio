...
# GANTI KE FOLDER NO AUGMENT
DATA_DIR = PROJECT_DIR / 'data' / 'processed_no_augment' 
MLRUNS_DIR = PROJECT_DIR / 'mlruns'
MODEL_SAVE_DIR = PROJECT_DIR / 'cnn_experiment' / 'models_basic_no_augment'
...
def plot_cm(y_true, y_pred, classes):
    ...
    plt.savefig("cm_no_augment.png")
    plt.show()

def main():
    if not MLRUNS_DIR.exists(): MLRUNS_DIR.mkdir(parents=True, exist_ok=True)
    mlflow.set_tracking_uri(f"file:///{MLRUNS_DIR.as_posix()}")
    # GANTI NAMA EKSPERIMEN BIAR RAPI
    mlflow.set_experiment("Basic_Local_No_Augmentation")
    mlflow.tensorflow.autolog()

    with mlflow.start_run():
        # 1. Log Parameter Eksperimen (Sangat Penting untuk Skripsi)
        mlflow.log_param("dataset_path", str(DATA_DIR))
        mlflow.log_param("data_split", "72 Train / 28 Test")
        mlflow.log_param("augmentation", "FALSE (None)") # Penanda Utama
        mlflow.log_param("features", "Global Mean Pooling")
        
        yamnet = load_yamnet()
        print("📥 Loading Dataset (No Augment)...")
        X_train, y_train_txt, train_files = load_subset(yamnet, 'train')
        X_test, y_test_txt, test_files = load_subset(yamnet, 'test')
        
        # 2. Upload Daftar File (Bukti Audit)
        manifest_path = "dataset_manifest_no_aug.txt"
        with open(manifest_path, "w") as f:
            f.write("=== TRAINING DATA (Pure Original) ===\n")
            for i, fname in enumerate(train_files):
                f.write(f"{y_train_txt[i]} : {fname}\n")
            f.write("\n=== TESTING DATA (Pure Original) ===\n")
        ...
        # 3. Model Arsitektur (SAMA PERSIS DENGAN SEBELUMNYA)
        model = Sequential([
        ...
        print("🚀 Mulai Training (Tanpa Augmentasi)...")
        # Perhatikan: Jumlah batch/step akan lebih sedikit karena data cuma 72
        model.fit(X_train, y_train, epochs=100, batch_size=16, 
                  validation_data=(X_test, y_test), callbacks=[early_stop])

        # 4. Evaluasi
        loss, acc = model.evaluate(X_test, y_test, verbose=0)
        print(f"\n🏆 No-Augment Model Accuracy: {acc*100:.2f}%")
        ...
        mlflow.log_artifact("cm_no_augment.png")
        
        # Simpan
        if not MODEL_SAVE_DIR.exists(): MODEL_SAVE_DIR.mkdir(parents=True, exist_ok=True)
        model.save(MODEL_SAVE_DIR / 'model_no_augment.h5')
